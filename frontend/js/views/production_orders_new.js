        ...
        try {
            ...
            const designPages = Object.values(itemsWithFiles).map(({ item: i, files }) => {
                ...
                const fileSections = files.map((f, fIdx) => {
                    ...
                    let previewMarkup = '';
                    const thumbUrl = _normalizeDesignUrl(i.design_thumbnail);
                    const thumbAbsolute = thumbUrl.startsWith('http') ? thumbUrl : window.location.origin + thumbUrl;
                    if (isImage || (thumbUrl && !isPdf)) {
                        previewMarkup = `<div class="pdf-preview-wrap" style="background:#f8fafc;min-height:auto;padding:16px;">
                            <img src="${thumbAbsolute || absoluteUrl}" alt="${_escapeHtml(fileName)}" class="design-img" style="max-height:500px;" onerror="this.onerror=null;this.parentElement.innerHTML='<div style=\'display:flex;flex-direction:column;align-items:center;justify-content:center;padding:40px;\'><div style=\'width:64px;height:64px;border-radius:14px;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;margin-bottom:12px;color:${fileMeta.color};background:${fileMeta.bg};\'>${fileMeta.label}</div><p style=\'font-size:12px;color:#94a3b8;\'>${fileMeta.full}</p></div>';">
                        </div>`;
                    } else if (isPdf) {
                        previewMarkup = `<div class="pdf-preview-wrap" style="background:#f8fafc;min-height:auto;padding:24px;">
                            <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;">
                                ${thumbUrl ? `<img src="${thumbAbsolute}" alt="${_escapeHtml(fileName)}" class="design-img" style="max-height:400px;margin-bottom:16px;border-radius:8px;border:1px solid #e2e8f0;" onerror="this.style.display='none';">` : ''}
                                ...
                                <p style="font-size:12px;color:#64748b;">${fileMeta.full} — استخدم زر التحميل بالأسفل لفتح الملف</p>
                            </div>
                        </div>`;
                    } else {
                        previewMarkup = `<div class="pdf-preview-wrap" style="background:#f8fafc;min-height:auto;padding:24px;">
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .pdf-preview-wrap {
    ...
    width: 100%; overflow: auto; position: relative; background: #f8fafc; padding: 16px;
  }
