...
router.post('/revert-order/:orderId', restrictDelete, async (req, res) => {
    ...
    try {
        ...
        // Delete design activity log (table has immutability triggers — disable temporarily)
        await client.query(`ALTER TABLE design_activity_log DISABLE TRIGGER prevent_delete_design_activity_log`);
        await client.query(
            `DELETE FROM design_activity_log WHERE order_id = $1`,
            [orderId]
        );
        await client.query(`ALTER TABLE design_activity_log ENABLE TRIGGER prevent_delete_design_activity_log`);
