...
router.put('/warehouses/:id', restrictEdit, validateBody(warehouseUpdate), async (req, res) => {
    ...
    try {
        const result = await db.withTransaction(async (client) => {
            // Get old client_id before update
            const oldRes = await client.query('SELECT client_id FROM warehouses WHERE id = $1', [req.params.id]);
            if (oldRes.rowCount === 0) return null;
            const oldClientId = oldRes.rows[0].client_id;
            ...
            const whRes = await client.query(
                `UPDATE warehouses SET
                 ...
                 WHERE id = $7
                 RETURNING *`,
                [name.trim(), code || null, warehouse_type || 'main', address || location || null, newClientId, status || 'active', req.params.id]
            );

            // If client_id changed, migrate stock to new client
            if (oldClientId !== newClientId) {
                await client.query(
                    `UPDATE warehouse_stock SET client_id = $1, last_updated = NOW()
                     WHERE warehouse_id = $2`,
                    [newClientId, req.params.id]
                );
            }
