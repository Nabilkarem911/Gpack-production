...
CREATE INDEX idx_client_designs_client_variant ON client_designs(client_id, variant_id);
CREATE INDEX idx_client_designs_variant ON client_designs(variant_id);
CREATE INDEX idx_client_designs_active ON client_designs(client_id, variant_id, is_active);

-- Table: client_design_files
-- Stores files for each design (thumbnail, pdf, ai, psd)
CREATE TABLE client_design_files (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_design_files_design ON client_design_files(design_id);
CREATE INDEX idx_design_files_type ON client_design_files(design_id, file_type);
